...
def save_to_file(complaint_name, complaint_address, complaint_mobile, identification_no, complaint_type, identification_type, detailed_location, accused_name, accused_address, complaint_detail, location_of_incident, incident_date, incident_time, submission_time, status, status_response):
    content = f"Complaint Name (As per Identification Card): {complaint_name}\nComplaint Address: {complaint_address}\nComplaint Mobile Number: {complaint_mobile}\nIdentification No.: {identification_no}\nComplaint Type: {complaint_type}\nIdentification Type: {identification_type}\nDetailed Location: {detailed_location}\nAccused Name (if known): {accused_name}\nAccused Address (if known): {accused_address}\nComplaint Detail: {complaint_detail}\nLocation of Incident (PS): {location_of_incident}\nDate of Incident: {incident_date}\nTime of Incident: {incident_time}\nSubmission Time: {submission_time}\nStatus: {status}\nStatus Response: {status_response}"
...
def submit_form():
    ...
    incident_date = incident_date_entry.get()
    incident_time = incident_time_spinbox.get()
    submission_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    status = status_var.get()
    status_response = status_response_entry.get()
    save_to_file(complaint_name, complaint_address, complaint_mobile, identification_no, complaint_type, identification_type, detailed_location, accused_name, accused_address, complaint_detail, location_of_incident, incident_date, incident_time, submission_time, status, status_response)
